...
read_sim_gff <- function(path) {
  ...
  rows <- lapply(lines, function(line) {
    ...
    data.frame(
      ...
      start                 = as.integer(f[4L]),   # GFF 1-based
      end                   = as.integer(f[5L]),
      strand                = f[7L],
...
for (row_i in seq_len(nrow(top_elements))) {
  ...
  sm <- build_site_matrix(el_row$pop_id, el_row$gen_id, el_row,
                           file_meta, flank_bp)
  if (is.null(sm)) {
...
p_ld <- ggplot(ld_summary,
  ...
  labs(
    ...
    title = sprintf("LD decay around top-%d selected elements (± %d bp)", top_n, flank_bp)
  ) +
...
p_heat <- ggplot(all_ld |> filter(!is.na(r2)),
  ...
  labs(
    ...
    title = "Pairwise LD heatmap (focal element sites vs locus)"
  ) +
